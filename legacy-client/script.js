...
// Server-side routing uses a single `portfolioTheme` cookie
// (light = OLD portfolio default, dark = NEW portfolio).
...
function applyThemeToggleIcons(theme) {
...
(function initThemeToggle() {
  ...
  applyThemeToggleIcons(theme);

  document.querySelectorAll('.theme-toggle').forEach(btn => {
    btn.addEventListener('click', () => {
      const next = theme === 'dark' ? 'light' : 'dark';
      document.cookie =
        'portfolioTheme=' + next + '; path=/; max-age=31536000; SameSite=Lax';
