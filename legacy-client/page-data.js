  ...
  try {
    ...
    if (bio) {
      ...
      bio.innerHTML = paras.length
        ? paras.map((p, i) => {
            const text = esc(p);
            // Replace the static experience token ("N months") with the live counter.
            const html = i === 0 ? insertExperienceCounterInto(text) : text;
            return `<p class="about-text visible">${html}</p>`;
          }).join('')
        : '';
      bio.querySelectorAll('.experience-counter').forEach(initExperienceCounter);
    }
...
async function initProjectsPage() {
  ...
  if (!clientContainer || !personalContainer) return;

  const loading = '<div style="padding:40px;text-align:center;color:var(--light-text);font-family:\'Orbitron\',sans-serif;font-size:0.8rem;letter-spacing:0.1em"><i class="fas fa-circle-notch fa-spin"></i> Loading…</div>';
  clientContainer.innerHTML   = loading;
  personalContainer.innerHTML = loading;
  ...
  try {
    ...
    clientContainer.innerHTML = client.length
      ? client.map(projectCardHTML).join('')
      : '<p style="color:var(--light-text);text-align:center;padding:40px">No client projects yet.</p>';

    personalContainer.innerHTML = personal.length
      ? personal.map(projectCardHTML).join('')
      : '<p style="color:var(--light-text);text-align:center;padding:40px">No personal projects yet.</p>';

    makeVisible(clientContainer);
    makeVisible(personalContainer);
  } catch (e) {
    const err = `<p style="color:var(--light-text);text-align:center">Error loading projects: ${e.message}</p>`;
    clientContainer.innerHTML   = err;
    personalContainer.innerHTML = err;
  }
}
